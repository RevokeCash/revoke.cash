...
CREATE TYPE "public"."premium_payment_intent_status" AS ENUM('pending', 'confirmed', 'expired', 'failed');--> statement-breakpoint
...
CREATE TABLE "premium_payment_intents" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL,
	...
	"status" "premium_payment_intent_status" NOT NULL,
...
CREATE TABLE "premium_subscriptions" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "premium_subscriptions_payment_intent_id_unique" UNIQUE("payment_intent_id")
);
--> statement-breakpoint
ALTER TABLE "premium_payment_intents" ADD CONSTRAINT "premium_payment_intents_plan_fk" FOREIGN KEY ("plan_id","plan_version") REFERENCES "public"."premium_plans"("id","version") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "premium_subscriptions" ADD CONSTRAINT "premium_subscriptions_payment_intent_id_premium_payment_intents_id_fk" FOREIGN KEY ("payment_intent_id") REFERENCES "public"."premium_payment_intents"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "idx_premium_payment_intents_owner" ON "premium_payment_intents" USING btree ("owner_address");--> statement-breakpoint
CREATE INDEX "idx_premium_payment_intents_status" ON "premium_payment_intents" USING btree ("status");--> statement-breakpoint
CREATE INDEX "idx_premium_payment_intents_chain_scan_start" ON "premium_payment_intents" USING btree ("chain_id","scan_from_block");--> statement-breakpoint
CREATE UNIQUE INDEX "idx_premium_payment_intents_matched_tx_hash_unique" ON "premium_payment_intents" USING btree ("matched_tx_hash") WHERE "premium_payment_intents"."matched_tx_hash" IS NOT NULL;--> statement-breakpoint
...
VALUES
  ('individual_annual', 1, 'Premium Individual', 99, 365, 1, true),
  ('bundle10_annual', 1, 'Premium Bundle (10)', 299, 365, 10, true)
