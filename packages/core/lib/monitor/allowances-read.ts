...
import {
  monitorAllowanceState,
  monitorScanState,
  type monitorTokenMetadata,
} from '@revoke.cash/core/db/schema/monitor';
...
import { getCachedTokenMetadata } from './token-enrichment';

type TokenMetadataRow = typeof monitorTokenMetadata.$inferSelect;
type ScanState = Pick<typeof monitorScanState.$inferSelect, 'consecutiveFailures' | 'lastError' | 'lastScanAt'>;
...
const getScanState = async (address: Address, chainId: DocumentedChainId): Promise<ScanState | undefined> => {
  return getDb().query.monitorScanState.findFirst({
...
const getAllowanceFailureState = async (address: Address, chainId: DocumentedChainId): Promise<FailureState> => {
  return getDb().query.monitorAllowanceState.findFirst({
