...
export const findAffectedTokens = async (
  ...
  const tokenFromAddress = sql<string>`${monitorEventsCache.address}`;
  const tokenFromTopic2 = sql<string>`'0x' || substring(${monitorEventsCache.topic2} from 27 for 40)`;
  const tokenFromData = sql<string>`'0x' || substring(${monitorEventsCache.data} from 27 for 40)`;
  ...
  const approvedTokensSubquery = db
    ...
    .where(
      and(
        ...
        eq(monitorEventsCache.topic1, userTopic),
    ...
    db
      ...
      .where(
        and(
          eq(monitorEventsCache.chainId, chainId),
          eq(monitorEventsCache.topic1, userTopic),
          inArray(monitorEventsCache.topic0, topic0s),
  ...
  const rowsByQuery = await Promise.all([
    ...
    distinct(tokenFromAddress, [ERC721_TRANSFER_TOPIC], inArray(monitorEventsCache.address, approvedTokensSubquery)),
